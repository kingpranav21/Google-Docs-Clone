...
export default function OpenWord() {
  ...
  // BUTTON: Page Break (blue button, or Ctrl+Enter)
  // Creates a new page and splits content at cursor position
  // Content before cursor stays on current page, content after moves to new page
  const insertPageBreak = (targetIndex = currentPage) => {
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      rangeAfter.selectNodeContents(editor);
      rangeAfter.setStart(range.endContainer, range.endOffset);
      const fragmentAfter = rangeAfter.extractContents(); // removes from DOM

      // Content before cursor is what remains in editor
      contentBefore = editor.innerHTML;

      // Create temporary div for content after cursor
      const tempDiv = document.createElement('div');
      tempDiv.appendChild(fragmentAfter);
      contentAfter = tempDiv.innerHTML;
    } else {
      // No cursor position, just split at end
      contentBefore = editor.innerHTML;
      contentAfter = '';
    }
    ...
    newPages[targetIndex].content = contentBefore;

    // Create new page with content after cursor
    const newPage = {
      id: pages.length + 1,
      content: contentAfter,
      header: pages[0]?.header || '', // Copy shared header
      footer: pages[0]?.footer || '', // Copy shared footer
    };
  ...
  const splitOverflow = (pageIndex, depth = 0) => {
    // Safety guard to prevent infinite recursion
    if (depth > 10) return;
    ...
    // Ensure next page exists before moving content
    ...
    // Move the last block-level node (or last child) to the next page
    const moveOneBlock = () => {
    ...
    // Move blocks until current page is within limit or no blocks left
    let moved = false;
    while (isOverflowing()) {
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-300 p-3 sticky top-0 z-10 shadow-sm">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {/* Page Controls - Page Break, Header/Footer Editing */}
          <div className="flex gap-2 border-l pl-2 ml-auto">
            {/* Page Break Button - Splits content at cursor and creates new page */}
            <button
              onClick={insertPageBreak}
              className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm"
              title="Insert Page Break (Ctrl+Enter) - Splits content at cursor position"
            >
              Page Break
            </button>
